...
from data_pcd import OfflinePcdDataset
# from data_col import ColDataset
from model_p2v import P2VModel
...
class Config(WandbConfig, HfConfig, ConfigBase):
    ...
    dataset: OfflinePcdDataset.Config = OfflinePcdDataset.Config(
    ...
    # dataset = ColDataset(ColDataset.Config('/tmp/col-12-2048',
    #                                        preload=True))

    # use_wds: bool = True
    # wds_cache: str = '/home/user/.cache/pkm/dyn-v12/'

    model: P2VModel.Config = rrm(
        P2VModel.Config(),
        {
            # == dynmodel ==
            'tau': 0.0001,
    ...
    cos_cycle_steps: int = num_epoch
    # Maybe this needs to be longer
    cos_warmup_steps: int = 80
...
def load_dataset(cfg, aux=None):
    ...
    train_dataset = OfflinePcdDataset(cfg.dataset,
                                      'train',
    ...
    valid_dataset_cfg = replace(cfg.dataset, shuffle=False)
    valid_dataset = OfflinePcdDataset(valid_dataset_cfg,
                                      'valid',
...
def learn(cfg: Config):
    ...
    model = P2VModel(cfg.model).to(device=device)
