...
from data_rot import RotDataset
from model_rot import RotModel
...
class Config(WandbConfig, HfConfig, ConfigBase):
    ...
    # == wandb ==
    project: Optional[str] = 'corn-/rot'
    sync_tensorboard: bool = True
    save_code: bool = True
    use_wandb: bool = True
    # It's technically "optional", but required if
    # `use_wandb`=True.
    group: Optional[str] = '6DMseLoss-col12'
    ...
    dataset: RotDataset.Config = RotDataset.Config(
        data_root='/tmp/docker/col-12-2048/',
        # data_root='/tmp/col10/',
        preload=False,
        device='cpu'
...
def load_dataset(cfg, aux=None):
    if cfg.use_wds:
        ...
        train_dataset = (
            as_wds(F'{cfg.wds_cache}/train',
                   partial(RotDataset, data_cfg, split='train'),
        ...
        train_transform = Preprocess(cfg.process, 'cpu')
        train_dataset = RotDataset(cfg.dataset,
                                   'train',
    ...
    if cfg.use_wds:
        ...
        valid_dataset = as_wds(
            ...
            partial(RotDataset, data_cfg, split='valid'),
        ...
        valid_transform = Preprocess(replace(cfg.process,
                                             ...
                                             noise_scale=0.0),
                                     'cpu')
        valid_dataset = RotDataset(cfg.dataset,
                                   'valid',
...
def learn(cfg: Config):
    ...
    device: str = cfg.device
    model = RotModel(cfg.model).to(device=device)
    train_loader, valid_loader = load_dataset(cfg)
