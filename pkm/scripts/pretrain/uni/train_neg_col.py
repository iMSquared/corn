...
from data_neg_col import NegColDataset
from model_neg_col import NegColModel
...
class Config(WandbConfig, HfConfig, ConfigBase):
    ...
    # == wandb ==
    project: Optional[str] = 'corn-/neg-col'
    ...
    dataset: NegColDataset.Config = NegColDataset.Config(
        data_root='/input/genom/col-with-empty/',
        # FIXME: ONLY FOR TESTING
        # max_count=128,
    )

    use_wds: bool = True
    wds_cache: str = '/home/user/.cache/pkm/col-with-empty/'

    model: NegColModel.Config = rrm(
        NegColModel.Config(),
        {
            # == negcolmodel ==
            'mask_ratio': 0.75,
...
def load_dataset(cfg, aux=None):
    if cfg.use_wds:
        ...
        train_dataset = (
            as_wds(F'{cfg.wds_cache}/train',
                   partial(NegColDataset, data_cfg, split='train'),
        ...
        train_dataset = NegColDataset(cfg.dataset,
                                      'train',
    ...
    if cfg.use_wds:
        ...
        valid_dataset = as_wds(
            ...
            partial(NegColDataset, data_cfg, split='valid'),
        ...
        valid_transform = Preprocess(replace(cfg.process,
                                             ...
                                             noise_scale=0.0),
                                     cfg.device)
        valid_dataset = NegColDataset(cfg.dataset,
                                      'valid',
...
def learn(cfg: Config):
    ...
    device: str = cfg.device
    model = NegColModel(cfg.model).to(device=device)
    train_loader, valid_loader = load_dataset(cfg)
