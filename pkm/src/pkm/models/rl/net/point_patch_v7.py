...
class PointPatchV7FeatNet(nn.Module, FeatureBase):
    """
    ...
    class PointPatchV7FeatNetConfig(FeatureBase.Config):
        ...
        group_type: str = 'fps'
        patch_type: str = 'knn'
        pos_embed_type: str = 'mlp'  # or "none"
    ...
    Config = PointPatchV7FeatNetConfig
    ...
    def _group(self, x: th.Tensor,
        ...
        # If `pos_embed is not None, it's safe to subtract the centers
        # otherwise, it must remain as the original patch.
        sub = (True if (self.pos_embed is not None) else False)
        ...
        with th.no_grad():
            if cfg.group_type == 'hilbert':
                ...
                # patch centers.
                p, c = self.group(x, center=center, sub=sub, aux=aux)
...
def main():
    ...
    ctx = {'a': th.randn((batch_size, 32),
                         device='cuda'), 'b': th.randn((batch_size, 16),
                                                       device='cuda')}
    num_query: int = 1
    cfg = PointPatchV7FeatNet.Config(
        dim_in=(512, 3),
        dim_out=256,
        pw_attn=PatchWiseAttentionFeatNet.Config(
            dim_in=(16, 64),
    ...
    model = PointPatchV7FeatNet(cfg).to('cuda')
    ic(model)
    y = model(x, ctx)
    print(y.shape)
